...
public class ReceptionLogin extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == b1) {
            ...
            try (Conn c = new Conn();
                 PreparedStatement pst = c.getConnection()
                        .prepareStatement("SELECT id, role FROM login WHERE username=? AND password=?")) {
                    ...
                    if (rs.next()) {
                        ...
                        if (!"receptionist".equalsIgnoreCase(role)) {
                            JOptionPane.showMessageDialog(this, 
                                "Access denied! Only receptionists can login here.", 
                                "Access Denied", JOptionPane.ERROR_MESSAGE);
                            return;
                        }

                        // Open Reception page
                        SwingUtilities.invokeLater(() -> new Reception(userId, username));
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(ReceptionLogin::new);
    }
}